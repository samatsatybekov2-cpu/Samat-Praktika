...
$em     = $p['cat_icon'] ?? '🛍️';
$rel    = getProducts(6, 0, $p['category_id']);
?><!DOCTYPE html>
<html lang="ru">
<head>
...
<div class="container">
  ...
  <div class="prod-layout">
    ...
    <div class="prod-info">
      ...
      <div class="prod-actions">
        <button class="btn btn-primary btn-lg" id="addCartBtn" data-pid="<?= $p['id'] ?>">
          🛒 В корзину
        </button>
        <button class="btn-wish <?= $inWish?'on':'' ?>" id="wishBtn" data-pid="<?= $p['id'] ?>">
          <?= $inWish?'❤️':'🤍' ?>
        </button>
      </div>
  ...
  <?php $rel = array_filter($rel, function($r) use ($id){ return $r['id'] != $id; });
  if ($rel): ?>
  <div style="margin-top:48px">
    ...
    <div class="pgrid">
      <?php foreach (array_slice($rel,0,4) as $r):
        $re   = $r['cat_icon'] ?? '🛍️';
        $rdisc = $r['old_price'] > 0 ? round((1-$r['price']/$r['old_price'])*100) : 0;
      ?>
      <div class="pcard-item" onclick="location.href='<?= BASE_URL ?>/product.php?id=<?= $r['id'] ?>'">
        <?php if ($rdisc > 0): ?><span class="disc-badge">-<?= $rdisc ?>%</span><?php endif; ?>
        ...
        <div class="pcard-item__body">
          ...
          <div class="prow"><span class="pprice"><?= number_format($r['price'],0,'.',' ') ?> ₸</span>
          <?php if ($r['old_price'] > 0): ?><span class="pold"><?= number_format($r['old_price'],0,'.',' ') ?> ₸</span><?php endif; ?></div>
        </div>
      </div>
      <?php endforeach; ?>
    </div>
...
document.getElementById('addCartBtn').onclick = function(){
  ...
  .then(r=>r.json()).then(d=>{
    ...
    if(d.success){setCartBadge(d.count);toast('Добавлено в корзину 🛒','ok');}
...
document.getElementById('wishBtn').onclick = function(){
  ...
  .then(r=>r.json()).then(d=>{
    if(d.auth){openAuth();return;}
    if(d.success){b.textContent=d.added?'❤️':'🤍';b.classList.toggle('on',d.added);setWishBadge(d.count);toast(d.added?'Добавлено в избранное':'Удалено из избранного');}
  });
};
</script>
